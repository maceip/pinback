...
| `build/generated/static_assets.c` | `xxd`-generated embedded UI bundle |
| `ui/app/index.html`, `ui/app/app.js` | shell, fence parser, DSML panel, workspace picker |
| `tests/support/fake-ds4-agent.c` | deterministic CLI that mimics real agent for tests |
| `scripts/qa/pinback-smoke` | black-box smoke against fake agent |
| `scripts/qa/pinback-e2e` | exercise against real ds4-agent |
...
`scripts/embed/gen-static-assets.sh` regenerates the embedded `*.h` from
`ui/app/` and `ui/app/vendor/`. Run it after every web change.
...
- Unit (`tests/`): workspace store atomic write+load+corruption,
  ...
  event log, util, http.
- Smoke (`scripts/qa/pinback-smoke`): create/activate/reset/switch-back,
  409 on inactive input, CSP+SSE+Shiki asset reachability,
  long-markdown render fixture.
- E2E (`scripts/qa/pinback-e2e`): real ds4-agent in two scratch
...
- **Session resume across workspace switches.** ds4-agent's
  ...
  that drives the interactive TUI loop. Tracked in `docs/backlog/todo-paint.md`.
- macOS `.app` wrapper, parallel agents, per-workspace model selection,
  ...
  the workspace primitive. All in `docs/backlog/todo-paint.md`.
