...
under `experiments/transport-probe/capture/`. If you are tempted to re-litigate the
...
scrollback. The reference algorithm is `experiments/transport-probe/vterm_ref.py`,
...
```
cd experiments/transport-probe
