...
| 3 | `tests/support/fake-ds4-agent.c` | Real. Works. Honors `/save` and `/switch` because *the fake* parses slash commands; the real binary in this mode does not. |
...
4. Marked the todo complete and put the limitation in
   `docs/backlog/todo-paint.md` as deferred work.
...
**`tests/support/fake-ds4-agent.c`** (low risk, ~30 net lines)
...
**`scripts/qa/pinback-smoke`** (low risk, ~10 net lines)

- If the fake gets pty support, the smoke harness can stay verbatim.
- Otherwise, swap one launch flag.

**`scripts/qa/pinback-e2e`** (low risk, ~30 net lines added)
...
- `docs/architecture/ARCHITECTURE_REDO.md` — strike the "deferred PTY supervisor"
  paragraph; add the pty diagram to the agent supervisor section.
- `docs/backlog/todo-paint.md` — delete item #1 (it's now item #0, shipped).
