...
bool pin_http_read_request(int fd, pin_request *r) {
    ...
    if (!line_end) { pin_buf_free(&raw); return false; }
    ...
    if (!sp1) { pin_buf_free(&raw); return false; }
    const char *sp2 = memchr(sp1 + 1, ' ', line_len - (size_t)(sp1 + 1 - p));
    if (!sp2) { pin_buf_free(&raw); return false; }

    r->method = dup_token(p, (size_t)(sp1 - p));
    if (!r->method) { pin_buf_free(&raw); return false; }
    ...
    if (qmark) {
        ...
        if (!r->path) { pin_buf_free(&raw); pin_request_free(r); return false; }
    }
...
bool pin_http_header(const pin_request *r, const char *name,
    ...
    if (!r || !r->headers_blob) return false;
...
bool pin_http_respond(int fd, const pin_request *r, int status,
    ...
    if (r) pin_buf_printf(&hdr, "X-Request-Id: %s\r\n", r->request_id);
    if (content_type)
    ...
    if (ok && body_len) ok = (send_all(fd, body, body_len) >= 0);
    pin_buf_free(&hdr);
    return ok;
}
...
bool pin_http_respond_text(int fd, const pin_request *r,
    ...
    return pin_http_respond(fd, r, status, "text/plain; charset=utf-8",
...
bool pin_http_sse_emit(int fd, long long id, const char *event,
    ...
    if (event && *event) {
        int m = snprintf(prefix + n, sizeof(prefix) - (size_t)n,
                         "event: %s\n", event);
        if (m < 0) return false;
        n += m;
    }
...
bool pin_ws_upgrade(int fd, const pin_request *r) {
    ...
    if (!pin_http_header(r, "Upgrade", &v, &vlen)) return false;
    if (vlen < 9 || strncasecmp(v, "websocket", 9) != 0) return false;
    if (!pin_http_header(r, "Connection", &v, &vlen)) return false;
    /* "Connection" can be a comma list: search for "upgrade" token. */
    {
        ...
        for (char *t = strtok(tmp, ",; "); t; t = strtok(NULL, ",; ")) {
            if (!strcasecmp(t, "upgrade")) { found = true; break; }
        }
        if (!found) return false;
    }
    if (!pin_http_header(r, "Sec-WebSocket-Version", &v, &vlen)) return false;
    if (vlen != 2 || memcmp(v, "13", 2) != 0) return false;

    const char *key = NULL; size_t klen = 0;
    if (!pin_http_header(r, "Sec-WebSocket-Key", &key, &klen)) return false;
    if (klen < 16 || klen > 64) return false;
    ...
    pin_buf_puts(&out, "\r\nServer: pinback\r\n");
    if (r) pin_buf_printf(&out, "X-Request-Id: %s\r\n", r->request_id);
    pin_buf_puts(&out, "\r\n");
...
bool pin_ws_read_frame(int fd, int *out_opcode,
    ...
    *out_opcode  = -1;
    ...
    uint8_t h[2];
    if (recv_n(fd, h, 2) != 2) return false;
    ...
    if (rsv != 0) return false;          /* reserved bits must be 0 */
    if (!fin) return false;              /* v1: no fragmentation */
    if (!mask) return false;             /* client->server MUST mask */
    if (op > 0xA || (op > 0x2 && op < 0x8)) return false;  /* reserved opcodes */

    if (plen == 126) {
        uint8_t e[2];
        if (recv_n(fd, e, 2) != 2) return false;
        plen = ((uint64_t)e[0] << 8) | e[1];
    } else if (plen == 127) {
        uint8_t e[8];
        if (recv_n(fd, e, 8) != 8) return false;
        plen = 0;
        for (int i = 0; i < 8; i++) plen = (plen << 8) | e[i];
    }

    bool is_ctrl = (op & 0x8) != 0;
    if (is_ctrl && plen > PIN_WS_MAX_CTRL_PAYLOAD) return false;
    if (plen > PIN_WS_MAX_PAYLOAD) return false;

    uint8_t mkey[4];
    if (recv_n(fd, mkey, 4) != 4) return false;
