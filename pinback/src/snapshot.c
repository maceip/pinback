 ...
 * spawn failure. */
static int run_git(const char *git_dir, const char *work_tree,
    ...
    if (pid == 0) {
        if (chdir(work_tree) != 0) _exit(126);
        ...
        int n = 0;
        argv[n++] = "git";
        for (int i = 0; args[i] && n < 63; i++) argv[n++] = args[i];
        argv[n] = NULL;
        execvp("git", (char *const *)argv);
        _exit(127);
    }
    close(inp[0]); close(outp[1]);
    if (in && in_len) { ssize_t w = write(inp[1], in, in_len); (void)w; }
    close(inp[1]);
    char buf[4096];
    ssize_t k;
    while ((k = read(outp[0], buf, sizeof(buf))) > 0)
        if (out) pin_buf_append(out, buf, (size_t)k);
    close(outp[0]);
    int st = 0;
    waitpid(pid, &st, 0);
...
static bool ensure_repo(const char *git_dir, const char *work_tree) {
    ...
    struct stat sb;
    if (stat(head, &sb) == 0) return true;
    const char *init[] = { "init", "-q", NULL };
    if (run_git(git_dir, work_tree, init, NULL, 0, NULL) != 0) return false;
...
bool pin_snapshot_begin(const char *git_dir, const char *work_tree) {
    if (!git_dir || !work_tree) return false;
    if (!ensure_repo(git_dir, work_tree)) return false;
    const char *add[] = { "add", "-A", NULL };
    run_git(git_dir, work_tree, add, NULL, 0, NULL);
    const char *commit[] = { "commit", "-q", "--allow-empty", "-m",
                             "turn-start", NULL };
...
bool pin_snapshot_diff(const char *git_dir, const char *work_tree,
                       pin_buf *out) {
    if (!git_dir || !work_tree || !out) return false;
    char head[4200];
    snprintf(head, sizeof(head), "%s/HEAD", git_dir);
    struct stat sb;
    if (stat(head, &sb) != 0) return false;     /* no baseline yet */
    const char *add[] = { "add", "-A", NULL };
    run_git(git_dir, work_tree, add, NULL, 0, NULL);
...
bool pin_snapshot_revert(const char *git_dir, const char *work_tree,
    ...
    if (!git_dir || !work_tree || !patch || patch_len == 0) {
        if (errbuf && errcap) snprintf(errbuf, errcap, "empty patch");
        return false;
    }
