...
void pin_buf_putc(pin_buf *b, char c) { pin_buf_append(b, &c, 1); }

void pin_buf_puts(pin_buf *b, const char *s) {
    if (s) pin_buf_append(b, s, strlen(s));
}
...
void *pin_xmalloc(size_t n) {
    void *p = malloc(n ? n : 1);
    if (!p) pin_die("pinback: out of memory");
    return p;
}
...
void *pin_xrealloc(void *p, size_t n) {
    ...
    if (!q) pin_die("pinback: out of memory");
    return q;
}
...
bool pin_text_sanitize(pin_buf *out, const char *in, size_t len, size_t cap) {
    ...
    bool ok = true;
    while (p < end) {
        if (out->len - before >= cap) { ok = false; break; }
        uint32_t cp;
...
void pin_json_strn(pin_buf *b, const char *s, size_t len) {
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        switch (c) {
        case '"':  pin_buf_puts(b, "\\\""); break;
        case '\\': pin_buf_puts(b, "\\\\"); break;
        case '\b': pin_buf_puts(b, "\\b");  break;
        case '\f': pin_buf_puts(b, "\\f");  break;
        case '\n': pin_buf_puts(b, "\\n");  break;
        case '\r': pin_buf_puts(b, "\\r");  break;
        case '\t': pin_buf_puts(b, "\\t");  break;
        default:
            if (c < 0x20) {
                char esc[7] = "\\u00..";
...
void pin_json_str(pin_buf *b, const char *s) {
    if (!s) { pin_buf_puts(b, "null"); return; }
...
static void utf8_put(pin_buf *b, uint32_t cp) { utf8_encode(b, cp); }
...
bool pin_json_parse_string(const char **p, char **out) {
    ...
    while (**p && **p != '"') {
        if (**p == '\\') {
            (*p)++;
            switch (**p) {
            case '"':  pin_buf_putc(&b, '"');  (*p)++; break;
            case '\\': pin_buf_putc(&b, '\\'); (*p)++; break;
            case '/':  pin_buf_putc(&b, '/');  (*p)++; break;
            case 'b':  pin_buf_putc(&b, '\b'); (*p)++; break;
            case 'f':  pin_buf_putc(&b, '\f'); (*p)++; break;
            case 'n':  pin_buf_putc(&b, '\n'); (*p)++; break;
            case 'r':  pin_buf_putc(&b, '\r'); (*p)++; break;
            case 't':  pin_buf_putc(&b, '\t'); (*p)++; break;
            case 'u': {
                (*p)++;
...
static bool skip_value_depth(const char **p, int depth) {
    ...
    if (**p == '{') {
        ...
        while (**p && **p != '}') {
            ...
            if (**p != ':') return false;
            (*p)++;
            if (!skip_value_depth(p, depth + 1)) return false;
            pin_json_ws(p);
            if (**p == ',') { (*p)++; pin_json_ws(p); continue; }
            if (**p != '}') return false;
        }
        if (**p != '}') return false;
        (*p)++;
...
bool pin_json_skip_value(const char **p) { return skip_value_depth(p, 0); }
...
bool pin_json_find_key(const char *obj, const char *key, const char **out) {
    ...
    while (*p) {
        ...
        if (*p == '}' || !*p) return false;
        char *k = NULL;
        if (!pin_json_parse_string(&p, &k)) return false;
        pin_json_ws(&p);
        ...
        free(k);
        if (!pin_json_skip_value(&p)) return false;
        pin_json_ws(&p);
        if (*p == ',') { p++; continue; }
        if (*p == '}') return false;
    }
...
static void sha1_transform(pin_sha1 *c, const uint8_t b[64]) {
    ...
    c->state[0] += a; c->state[1] += B; c->state[2] += C;
...
void pin_sha1_final(pin_sha1 *c, uint8_t out[20]) {
    ...
    while (c->used != 56) pin_sha1_update(c, &zero, 1);
    uint8_t len[8];
