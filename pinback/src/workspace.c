...
static char *slurp(const char *path, size_t *out_len) {
    ...
    while (got < cap) {
        ...
        if (n < 0) { if (errno == EINTR) continue; close(fd); free(buf); return NULL; }
        if (n == 0) break;
        got += (size_t)n;
    }
    close(fd);
...
static void persist_catalog(pin_workspace_store *s) {
    ...
    if (s->active_id) pin_json_str(&b, s->active_id);
    else              pin_buf_puts(&b, "null");
    ...
    if (!atomic_write(s->catalog, b.ptr, b.len)) {
        PIN_LOG_WARNF("workspace.persist_fail", "path=%s errno=%d",
                      s->catalog, errno);
    }
    pin_buf_free(&b);
}
...
static void rec_free(ws_rec *r) {
    if (!r) return;
    if (r->log) pin_event_log_close(r->log);
...
void pin_workspace_meta_free(pin_workspace_meta *m) {
    ...
    free(m->label);  m->label = NULL;
    free(m->session_sha); m->session_sha = NULL;
}

void pin_workspace_meta_array_free(pin_workspace_meta *arr, size_t n) {
    if (!arr) return;
    for (size_t i = 0; i < n; i++) pin_workspace_meta_free(&arr[i]);
    free(arr);
}
...
static char *parse_string_field(const char *obj, const char *key) {
    const char *vp = NULL;
    if (!pin_json_find_key(obj, key, &vp)) return NULL;
    pin_json_ws(&vp);
    if (*vp == 'n') return NULL;       /* null */
    char *out = NULL;
    if (!pin_json_parse_string(&vp, &out)) return NULL;
    return out;
}
...
static long long parse_int_field(const char *obj, const char *key) {
    const char *vp = NULL;
    long long v = 0;
    if (!pin_json_find_key(obj, key, &vp)) return 0;
    if (!pin_json_parse_int(&vp, &v)) return 0;
    return v;
}
...
static void load_catalog(pin_workspace_store *s) {
    ...
    if (active) { free(s->active_id); s->active_id = active; }

    /* Find the workspaces array. */
    const char *vp = NULL;
    if (!pin_json_find_key(raw, "workspaces", &vp)) { free(raw); return; }
    pin_json_ws(&vp);
    if (*vp != '[') { free(raw); return; }
    vp++;
    pin_json_ws(&vp);
    while (*vp && *vp != ']') {
        pin_json_ws(&vp);
        ...
        if (!pin_json_skip_value(&vp)) break;
...
pin_workspace_store *pin_workspace_store_open(const char *root_dir,
    ...
    if (per_ws_ring_cap == 0) per_ws_ring_cap = 4096;
    if (!ensure_dir_recursive(root_dir, 0700)) {
        PIN_LOG_ERRF("workspace.root_fail", "root=%s errno=%d",
                     root_dir, errno);
        return NULL;
    }
    char ws_root[4096];
...
void pin_workspace_store_close(pin_workspace_store *s) {
    ...
    for (size_t i = 0; i < s->count; i++) rec_free(s->records[i]);
    free(s->records);
    free(s->active_id);
    free(s->root);
    free(s->catalog);
...
static void copy_meta(pin_workspace_meta *dst, const pin_workspace_meta *src) {
    ...
    if (src->id)          dst->id          = pin_xstrdup(src->id);
    if (src->path)        dst->path        = pin_xstrdup(src->path);
    if (src->label)       dst->label       = pin_xstrdup(src->label);
    if (src->session_sha) dst->session_sha = pin_xstrdup(src->session_sha);
...
bool pin_workspace_store_get(pin_workspace_store *s, const char *id,
    ...
    if (!s || !id || !out) return false;
    pthread_rwlock_rdlock(&s->rw);
    ws_rec *r = find_rec_locked(s, id);
    bool ok = false;
    if (r) { copy_meta(out, &r->meta); ok = true; }
    pthread_rwlock_unlock(&s->rw);
    return ok;
}
...
static char *basename_dup(const char *path) {
    if (!path) return pin_xstrdup("workspace");
...
bool pin_workspace_store_create(pin_workspace_store *s,
    ...
    if (!s || !path || !*path) {
        if (out_err) *out_err = pin_xstrdup("path required");
        return false;
    }
    if (path[0] != '/') {
        if (out_err) *out_err = pin_xstrdup("path must be absolute");
        return false;
    }
    /* Reject embedded NUL or .. for safety. */
    if (strstr(path, "/..") || strstr(path, "../")) {
        if (out_err) *out_err = pin_xstrdup("path must not contain '..'");
        return false;
    }
    struct stat st;
    if (stat(path, &st) != 0 || !S_ISDIR(st.st_mode)) {
        if (out_err) *out_err = pin_xstrdup("path is not a directory");
        return false;
    }
    ...
    for (size_t i = 0; i < s->count; i++) {
        if (s->records[i]->meta.path &&
            strcmp(s->records[i]->meta.path, path) == 0) {
            pthread_rwlock_unlock(&s->rw);
            if (out_err) *out_err = pin_xstrdup("workspace already exists for this path");
            return false;
        }
    }
    ws_rec *r = pin_xcalloc(1, sizeof(*r));
    ...
    if (out_id) *out_id = pin_xstrdup(r->meta.id);
    pthread_rwlock_unlock(&s->rw);
    return true;
}
...
bool pin_workspace_store_delete(pin_workspace_store *s, const char *id,
    ...
    if (out_err) *out_err = NULL;
    if (!s || !id) {
        if (out_err) *out_err = pin_xstrdup("id required");
        return false;
    }
    pthread_rwlock_wrlock(&s->rw);
    if (s->active_id && !strcmp(s->active_id, id)) {
        pthread_rwlock_unlock(&s->rw);
        if (out_err) *out_err = pin_xstrdup("workspace is active; deactivate first");
        return false;
    }
    size_t hit = (size_t)-1;
    for (size_t i = 0; i < s->count; i++) {
        if (s->records[i]->meta.id && !strcmp(s->records[i]->meta.id, id)) { hit = i; break; }
    }
    if (hit == (size_t)-1) {
        pthread_rwlock_unlock(&s->rw);
    ...
    rec_free(r);
    /* Compact array. */
    for (size_t i = hit; i + 1 < s->count; i++) s->records[i] = s->records[i + 1];
    s->count--;
    persist_catalog(s);
    pthread_rwlock_unlock(&s->rw);
    return true;
...
bool pin_workspace_store_set_active(pin_workspace_store *s, const char *id) {
    ...
    if (id) {
        ws_rec *r = find_rec_locked(s, id);
        if (!r) { pthread_rwlock_unlock(&s->rw); return false; }
...
bool pin_workspace_store_reset(pin_workspace_store *s, const char *id,
    ...
    if (r->log) { pin_event_log_close(r->log); r->log = NULL; }
    char log_path[4096];
    snprintf(log_path, sizeof(log_path), "%s/workspaces/%s/events.log",
             s->root, r->meta.id);
...
pin_event_log *pin_workspace_store_event_log(pin_workspace_store *s,
    ...
    pthread_rwlock_wrlock(&s->rw);
    ws_rec *r = find_rec_locked(s, id);
    if (!r) { pthread_rwlock_unlock(&s->rw); return NULL; }
    if (!r->log) {
        char log_path[4096];
        snprintf(log_path, sizeof(log_path), "%s/workspaces/%s/events.log",
                 s->root, r->meta.id);
