...
|----------|----------------|----------------|------------------|-------|
| macOS    | AppKit | WebKit / WKWebView | WKWebView (ObjC) | SwiftPM |
...
`.github/workflows/platform-shells.yml` compiles the **Linux**, **Windows**,
**Android**, and **macOS** shells on native GitHub runners, so they get real build
coverage without a local toolchain. **iOS** is not built in CI: it needs the iOS 26
SDK (Xcode 26 beta) for the new SwiftUI `WebView`, which hosted runners don't ship
yet — build it locally with Xcode 26.
