...
{
    switch (s) {
    case PIN_AGENT_STATE_IDLE:
        return "idle";
    case PIN_AGENT_STATE_SPAWNING:
        return "spawning";
    case PIN_AGENT_STATE_READY:
        return "ready";
    case PIN_AGENT_STATE_BUSY:
        return "busy";
    case PIN_AGENT_STATE_DRAINING:
        return "draining";
    case PIN_AGENT_STATE_DEAD:
        return "dead";
    }
    return "unknown";
}
...
{
    const char *dir = a->cfg.kvcache_dir;
    if (dir && *dir) {
        snprintf(a->kvcache_path, sizeof(a->kvcache_path), "%s", dir);
        return;
    }
    const char *home = getenv("HOME");
    if (!home || !*home)
        home = ".";
    snprintf(a->kvcache_path, sizeof(a->kvcache_path), "%s/.ds4/kvcache", home);
}

static pin_agent_state agent_state_get(pin_agent *a)
{
    pthread_mutex_lock(&a->mu);
    pin_agent_state s = a->state;
    pthread_mutex_unlock(&a->mu);
    return s;
}

static void timespec_deadline_ms(struct timespec *ts, long long deadline_ms)
{
    clock_gettime(CLOCK_REALTIME, ts);
    long long now_ms = (long long)pin_monotonic_ms();
...
/* Caller holds a->mu. Wait until the TUI reports booted (first idle). */
static bool kv_wait_boot_locked(pin_agent *a, long long deadline_ms)
{
    while (!a->booted && (long long)pin_monotonic_ms() < deadline_ms) {
        struct timespec ts;
        timespec_deadline_ms(&ts, deadline_ms);
        if (pthread_cond_timedwait(&a->cv, &a->mu, &ts) != 0 &&
            (long long)pin_monotonic_ms() >= deadline_ms)
            break;
    }
    return a->booted;
}

/* Caller holds a->mu. After boot, wait for sysprompt prefill (non-idle then idle). */
static bool kv_wait_prefill_idle_locked(pin_agent *a, long long deadline_ms)
{
    bool saw_busy = false;
    while ((long long)pin_monotonic_ms() < deadline_ms) {
        char st[32];
        pin_vterm_status(a->vt, st, sizeof(st));
        bool idle = st[0] && strcmp(st, "idle") == 0;
        if (!idle)
            saw_busy = true;
        if (saw_busy && idle)
            return true;
    ...
    char st[32];
    pin_vterm_status(a->vt, st, sizeof(st));
    return saw_busy && st[0] && strcmp(st, "idle") == 0;
}

/* Caller holds a->mu. Wait for /switch "session history" ack in stdout scan. */
static bool kv_wait_switch_locked(pin_agent *a, long long deadline_ms)
{
    while (!a->switch_restored && (long long)pin_monotonic_ms() < deadline_ms) {
        struct timespec ts;
        timespec_deadline_ms(&ts, deadline_ms);
        if (pthread_cond_timedwait(&a->cv, &a->mu, &ts) != 0 &&
            (long long)pin_monotonic_ms() >= deadline_ms)
...
static void emit_turn_diff(pin_agent *a, const char *diff, size_t len, int nfiles)
{
    pin_buf b;
    pin_buf_init(&b);
    pin_buf_putc(&b, '{');
...
/* Mark the supervisor READY and emit answer_end. Called from reader
 * threads; takes a->mu briefly (activate waits on cv while holding mu,
 * so readers can still make progress). */
...
static void sha_scan_after(pin_agent *a, const char *line, size_t n, const char *prefix)
{
    size_t plen = strlen(prefix);
    if (n < plen || memcmp(line, prefix, plen) != 0)
        return;
    size_t i = plen;
    while (i < n && line[i] == ' ')
        i++;
...
static void classifier_emit_line(pin_agent *a, const char *s, size_t n, bool had_newline)
{
    if (a->cfg.kv_resume) {
        ...
        if (n >= 2 && s[0] == '*' && s[1] == ' ')
            return;
        if (agent_state_get(a) != PIN_AGENT_STATE_BUSY)
            return;
    }
...
static void trace_cb_turn(void *ud)
{
    (void)ud;
}
...
static void *trace_reader_fn(void *arg)
{
    ...
    if (fd >= 0) {
        pin_tracestream_cb cb = {
            .ud = a,
            .on_answer = trace_cb_answer,
            .on_thinking = trace_cb_thinking,
            .on_tool_call = trace_cb_tool,
            .on_turn = trace_cb_turn,
        };
    ...
    if (ts) {
        if (line.len)
            pin_tracestream_feed_line(ts, line.ptr, line.len);
        pin_tracestream_flush(ts);
...
static void tui_process(pin_agent *a, const char *buf, size_t n, bool flush)
{
    ...
    if (n > 0) {
        char scan[160 + 4096];
        size_t cl =
            a->sha_carry_len > sizeof(a->sha_carry) ? sizeof(a->sha_carry) : a->sha_carry_len;
        memcpy(scan, a->sha_carry, cl);
        size_t take = n < 4096 ? n : 4096;
        memcpy(scan + cl, buf, take);
        size_t tot = cl + take;
        capture_sha_in_buf(a, scan, tot);
        if (memmem(scan, tot, "session history", 15)) {
            pthread_mutex_lock(&a->mu);
            a->switch_restored = true;
            pthread_cond_broadcast(&a->cv);
            pthread_mutex_unlock(&a->mu);
        }
    ...
    bool signal_turn_end = false;
    pthread_mutex_lock(&a->mu);
    if (nonidle)
        a->saw_turn_activity = true;
    ...
    if (!nonidle && a->saw_turn_activity && a->state == PIN_AGENT_STATE_BUSY) {
        ...
        signal_turn_end = true;
    }
    if (nonidle || signal_turn_end)
        pthread_cond_broadcast(&a->cv);
    pthread_mutex_unlock(&a->mu);
    (void)flush;
    if (signal_turn_end)
...
static bool spawn_child_locked(pin_agent *a, const char *path)
{
                ...
                if (slash) {
                    *slash = '\0';
                    ...
                    for (size_t i = 0; i < sizeof(metal_map) / sizeof(metal_map[0]); i++) {
                        if (getenv(metal_map[i].env))
                            continue;
                        snprintf(buf, sizeof(buf), "%s/metal/%s", real, metal_map[i].file);
    ...
    if (a->cfg.kv_resume) {
        if (a->vt)
            pin_vterm_free(a->vt); /* drop the previous spawn's screen */
    ...
    reader_args *ra1 = pin_xcalloc(1, sizeof(*ra1));
    ra1->a = a;
    reader_args *ra2 = pin_xcalloc(1, sizeof(*ra2));
    ra2->a = a;
...
static void close_child_io(pin_agent *a)
{
    ...
    if (a->child_stdout >= 0) {
        close(a->child_stdout);
        a->child_stdout = -1;
    }
    if (a->child_stderr >= 0) {
        close(a->child_stderr);
        a->child_stderr = -1;
    }
}
...
static bool stdin_write_line(pin_agent *a, const char *line)
{
    ...
    if (ok && write(a->child_stdin, &term, 1) < 0)
        ok = false;
    pthread_mutex_unlock(&a->stdin_mu);
    return ok;
}
...
static void build_pending_resume(pin_agent *a, const char *workspace_id)
{
    ...
    pin_buf b;
    pin_buf_init(&b);
    pin_buf_puts(&b, "[Resuming an earlier session in this workspace. Our conversation so "
...
pin_agent *pin_agent_new(const pin_agent_config *cfg, pin_workspace_store *ws)
{
    ...
    a->cfg = *cfg;
    if (a->cfg.spawn_ready_ms <= 0)
        a->cfg.spawn_ready_ms = 5000;
    if (a->cfg.term_timeout_ms <= 0)
        a->cfg.term_timeout_ms = 3000;
    agent_resolve_kvcache_path(a);
    a->ws = ws;
...
static bool newest_saved_session(pin_agent *a, time_t since, char *out, size_t cap)
{
    DIR *d = opendir(a->kvcache_path);
    if (!d)
        return false;
    time_t newest = 0;
    bool found = false;
    ...
    while ((e = readdir(d))) {
        ...
        if (L < 4 || strcmp(e->d_name + L - 3, ".kv") != 0)
            continue;
        if (strncmp(e->d_name, "sysprompt", 9) == 0)
            continue;
        char fp[1200];
        snprintf(fp, sizeof(fp), "%s/%s", a->kvcache_path, e->d_name);
        struct stat st;
...
bool pin_agent_activate(pin_agent *a, const char *workspace_id, char **out_err)
{
    ...
    if (a->cfg.kv_resume && meta.session_sha && *meta.session_sha) {
        long boot_ms = a->cfg.spawn_ready_ms > 0 ? a->cfg.spawn_ready_ms : 30000;
        long long boot_deadline = (long long)pin_monotonic_ms() + boot_ms;
        if (!kv_wait_boot_locked(a, boot_deadline)) {
            PIN_LOG_WARNF("kv.boot_timeout", "workspace=%s ms=%ld", meta.id, boot_ms);
        }
        long long prefill_deadline =
            (long long)pin_monotonic_ms() + (boot_ms > 5000 ? boot_ms : 5000);
        if (!kv_wait_prefill_idle_locked(a, prefill_deadline)) {
...
bool pin_agent_submit(pin_agent *a, const char *workspace_id, const char *user_text, char **out_err)
{
    ...
    pthread_mutex_lock(&a->mu);
    if (workspace_id && a->active_id[0] && strcmp(workspace_id, a->active_id) != 0) {
        pthread_mutex_unlock(&a->mu);
        if (out_err)
...
void pin_agent_status_get(pin_agent *a, pin_agent_status *out)
{
    ...
    pthread_mutex_lock(&a->mu);
    out->state = a->state;
    snprintf(out->active_workspace_id, sizeof(out->active_workspace_id), "%s", a->active_id);
    snprintf(out->active_workspace_path, sizeof(out->active_workspace_path), "%s", a->active_path);
