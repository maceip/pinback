...
static void format_event_json(long long seq, long long generation, const char *kind,
                              const char *raw_payload, size_t raw_payload_len, char **out,
                              size_t *out_len)
{
    pin_buf b;
    pin_buf_init(&b);
...
static void subs_unregister_unlocked(pin_event_log *log, pin_subscriber *s)
{
    if (s->prev)
        ...
        log->subs_head = s->next;
    if (s->next)
        s->next->prev = s->prev;
    else
        log->subs_tail = s->prev;
    log->subs_count--;
}
            ...
            if (fsize > 0) {
                ...
                if (got > 0) {
                    ...
                    while (line && *line) {
                        ...
                        if (llen > 0 && line[0] == '{' &&
                            pin_json_find_key(line, "generation", &vp) &&
...
void pin_event_log_close(pin_event_log *log)
{
    ...
    for (size_t i = 0; i < log->cap; i++)
        slot_free(&log->ring[i]);
    free(log->ring);
...
void pin_event_log_append(pin_event_log *log, const char *kind, const char *json, size_t json_len)
{
    if (!log)
        return;
    pthread_rwlock_wrlock(&log->rw);
    ...
    size_t event_len = 0;
    format_event_json(seq, log->generation, kind, json, json_len, &event_json, &event_len);
    /* Push into ring. */
    ring_slot *slot = &log->ring[log->head];
    if (slot->json)
        slot_free(slot);
...
void pin_event_log_bump_generation(pin_event_log *log)
{
    ...
    char *event_json = NULL;
    size_t event_len = 0;
    format_event_json(seq, log->generation, "cursor_reset", payload.ptr, payload.len, &event_json,
                      &event_len);
...
void pin_event_log_render_snapshot(pin_event_log *log, pin_buf *out)
{
    ...
    pin_buf_printf(out, ",\"oldest_seq\":%lld", log->newest_seq == 0 ? 0 : log->oldest_seq);
...
{
    const char *pp = NULL;
    if (!pin_json_find_key(json, "payload", &pp))
        return NULL;
    const char *tp = NULL;
    if (!pin_json_find_key(pp, "text", &tp))
        return NULL;
    char *text = NULL;
    if (!pin_json_parse_string(&tp, &text))
        return NULL;
    return text;
}

void pin_event_log_last_preview(pin_event_log *log, pin_buf *user_out, pin_buf *answer_out)
{
    if (!log)
        return;
    pthread_rwlock_rdlock(&log->rw);
    ...
    for (size_t i = 0; i < log->used && !(have_user && have_ans); i++) {
        ...
        if (!json)
            continue;
        const char *kp = NULL;
        if (!pin_json_find_key(json, "kind", &kp))
            continue;
        char *kind = NULL;
        if (!pin_json_parse_string(&kp, &kind))
            continue;
        if (!have_user && !strcmp(kind, "user") && user_out) {
            char *t = event_payload_text(json);
            if (t) {
                pin_buf_puts(user_out, t);
                free(t);
                have_user = true;
            }
...
void pin_event_log_render_transcript(pin_event_log *log, pin_buf *out, size_t max_bytes)
{
    if (!log || !out)
        return;
    pin_buf full;
    ...
    for (size_t i = 0; i < log->used; i++) {
        ...
        if (!strcmp(kind, "user") && text) {
            ...
            if (strcmp(prev, "answer") != 0)
                pin_buf_puts(&full, "Assistant: ");
 ...
 * this value). */
static long long apply_resume(pin_event_log *log, pin_subscriber *s, long long client_generation,
                              long long after_seq)
{
    pthread_rwlock_rdlock(&log->rw);
    ...
    if (client_generation > 0 && client_generation != gen) {
        ...
        PIN_LOG_INFOF(PIN_EV_STREAM_CURSOR_RESET, "client_gen=%lld server_gen=%lld",
    ...
    if (after_seq == 0) {
        ...
        pthread_rwlock_unlock(&log->rw);
        PIN_LOG_INFOF(PIN_EV_STREAM_RESUME, "replay_all newest=%lld", newest);
        return newest;
    }
    ...
    if (after_seq > newest) {
        ...
        int n = snprintf(buf, sizeof(buf),
                         ...
                         gen, newest);
        sub_send_event(s, -1, buf, (size_t)n);
        PIN_LOG_INFOF(PIN_EV_STREAM_CURSOR_RESET, "after=%lld newest=%lld", after_seq, newest);
        return newest;
    }
    /* Inside ring -> replay. */
    if (after_seq >= oldest) {
        ...
        pthread_rwlock_unlock(&log->rw);
        PIN_LOG_INFOF(PIN_EV_STREAM_RESUME, "after=%lld newest=%lld", after_seq, newest);
        return newest;
    }
    /* Cursor older than ring -> snapshot. */
    {
        ...
        pthread_mutex_unlock(&s->fd_mu);
        pin_buf_free(&snap);
        PIN_LOG_INFOF(PIN_EV_STREAM_SNAPSHOT_REQUIRED, "after=%lld oldest=%lld", after_seq, oldest);
        return newest;
    }
}
...
void pin_event_log_serve_subscriber(pin_event_log *log, int fd, pin_sub_kind kind,
                                    long long client_generation, long long after_seq,
                                    pin_subscriber_callbacks cb)
{
    pin_subscriber *s = subs_register(log, fd, kind);
    PIN_LOG_INFOF(PIN_EV_STREAM_OPEN, "kind=%d after=%lld", (int)kind, after_seq);
    ...
    if (kind == PIN_SUB_KIND_SSE) {
        ...
        while (1) {
            ...
            size_t plen = 0;
            if (!pin_ws_read_frame(s->fd, &op, &payload, &plen))
                break;
            if (op == 0x8) { /* close */
                free(payload);
                break;
