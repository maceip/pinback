    ...
    for (;;) {
        ...
        if (!ts->answer_mode) {
            ...
            if (te) {
                ...
                if (pre && cb->on_thinking)
                    cb->on_thinking(cb->ud, acc->ptr, pre);
                consume(acc, pre + strlen(THINK_END));
                ts->answer_mode = true;
                continue;
            }
            size_t emit = acc->len > reserve ? acc->len - reserve : 0;
            if (emit && cb->on_thinking)
                cb->on_thinking(cb->ud, acc->ptr, emit);
            consume(acc, emit);
            break;
        }
        if (ts->in_dsml) {
            ...
            if (cl) {
                ...
                if (cb->on_tool_call)
                    cb->on_tool_call(cb->ud, acc->ptr, blk);
                consume(acc, blk);
                ts->in_dsml = false;
                continue;
            }
            if (flush) { /* unterminated block at stream end: emit as-is */
                if (cb->on_tool_call)
                    cb->on_tool_call(cb->ud, acc->ptr, acc->len);
                consume(acc, acc->len);
                ts->in_dsml = false;
            }
            break;
        ...
        if (op) {
            ...
            if (pre && cb->on_answer)
                cb->on_answer(cb->ud, acc->ptr, pre);
            consume(acc, pre); /* keep the open tag in acc */
            ts->in_dsml = true;
            continue;
        }
        size_t emit = acc->len > reserve ? acc->len - reserve : 0;
        if (emit && cb->on_answer)
            cb->on_answer(cb->ud, acc->ptr, emit);
        consume(acc, emit);
        break;
    }
}
...
{
    if (*len > 24 && s[4] == '-' && s[7] == '-' && s[10] == ' ' && s[13] == ':' && s[16] == ':') {
        const char *sp = memchr(s + 19, ' ', *len - 19);
        if (sp) {
            size_t adv = (size_t)(sp - s) + 1;
            *len -= adv;
            return s + adv;
        }
    }
...
void pin_tracestream_feed_line(pin_tracestream *ts, const char *line, size_t len)
{
    ...
    if (len >= 12 && memcmp(s, "tokens label", 12) == 0) {
        ...
        if (st)
            a = strtol(st + 6, NULL, 10);
        if (ln)
            b = strtol(ln + 4, NULL, 10);
        if (a >= 0 && b >= a)
            ts->skip = (int)(b - a);
        return;
    }
