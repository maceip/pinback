...
static bool is_editor_row(const char *row, int cols)
{
    ...
    tmp[n] = '\0';
    if (strstr(tmp, "ctx ") != NULL && strstr(tmp, "| ") != NULL)
        return true;
    /* one-time boot banner ds4-agent prints after the model loads */
    if (strstr(tmp, "DwarfStar Agent, context") != NULL)
        return true;
    return false;
}
...
{
    if (is_editor_row(vt->grid[r], vt->cols))
        return;
    int end = vt->cols;
    while (end > 0 && vt->grid[r][end - 1] == ' ')
        end--;
    if (end == 0)
        return; /* drop blank scrolled rows */
...
static int apply_csi(pin_vterm *vt, const char *body, size_t len)
{
    ...
    if (body[0] == '?')
        return final == 'n' ? 1 : 0; /* (no private CPR) */
    int n = 0;
    bool have = false;
    int nums[8];
    int nn = 0;
    ...
    switch (final) {
    case 'A':
        vt->cr = clampi(vt->cr - (p ? p : 1), 0, vt->rows - 1);
        break;
    case 'B':
        vt->cr = clampi(vt->cr + (p ? p : 1), 0, vt->rows - 1);
        break;
    case 'C':
        vt->cc = clampi(vt->cc + (p ? p : 1), 0, vt->cols - 1);
        break;
    case 'D':
        vt->cc = clampi(vt->cc - (p ? p : 1), 0, vt->cols - 1);
        break;
    case 'G':
        vt->cc = clampi((p ? p : 1) - 1, 0, vt->cols - 1);
        break;
    case 'H':
    case 'f': {
    ...
    case 'K':
        if (p == 0)
            for (int x = vt->cc; x < vt->cols; x++)
                vt->grid[vt->cr][x] = ' ';
        else if (p == 1)
            for (int x = 0; x <= vt->cc && x < vt->cols; x++)
                vt->grid[vt->cr][x] = ' ';
        else if (p == 2)
            row_clear(vt, vt->cr);
        break;
    ...
    case 'n':
        if (p == 6)
            return 1; /* CPR request */
        break;
    default:
        break; /* m (SGR) and anything else: ignore */
    }
    return 0;
}
    ...
    for (size_t i = 0; i < n; i++) {
        ...
        if (vt->esc == 1) {
            if (b == '[') {
                ...
                vt->esc = 0; /* ESC + non-[ : ignore */
            continue;
        }
        if (vt->esc == 2) {
            if (vt->csi_len < sizeof(vt->csi))
                vt->csi[vt->csi_len++] = (char)b;
            if (b >= 0x40 && b <= 0x7E) {
        ...
        if (b == 0x0D)
            ...
            vt->cc = clampi((vt->cc / 8 + 1) * 8, 0, vt->cols - 1);
        else if (b >= 0x20)
            put_byte(vt, b);
    }
    return cpr;
